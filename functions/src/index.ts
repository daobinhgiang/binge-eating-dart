...
function isValidFCMToken(token: any): boolean {
  return typeof token === 'string' && token.length > 0;
}

// Helper function to send FCM message with retry logic
async function sendFCMMessageWithRetry(message: any, maxRetries: number = 3): Promise<any> {
...
async function fetchProgressContext(userId: string): Promise<any> {
  try {
    // Get user data
    const userDoc = await admin.firestore().collection('users').doc(userId).get();
    if (!userDoc.exists) {
    ...
    const daysSinceStart = Math.floor((Date.now() - (userData?.createdAt || Date.now())) / (1000 * 60 * 60 * 24));
...
async function fetchEducationProgressContext(userId: string): Promise<any> {
  try {
    // Get user data
    const userDoc = await admin.firestore().collection('users').doc(userId).get();
    if (!userDoc.exists) {
    ...
    const recentLessons = completedLessons.filter(lesson => 
      lesson.completedAt && lesson.completedAt.toDate() > weekAgo
    );
...
async function fetchTodoContext(userId: string): Promise<any> {
  try {
    // Get user data
    const userDoc = await admin.firestore().collection('users').doc(userId).get();
    if (!userDoc.exists) {
    ...
    const todayTodos = allTodos.filter(todo => {
      const dueDate = todo.dueDate?.toDate() || new Date(todo.dueDate);
    ...
    const recentCompletedTodos = allTodos.filter(todo => {
      ...
      const completedAt = todo.completedAt?.toDate() || new Date(todo.completedAt);
...
async function generateAccountabilityMessage(todoContext: any, timeOfDay: string): Promise<string> {
  try {
...
${todoContext.pendingTodos.length > 0 ? `- Next tasks: ${todoContext.pendingTodos.map((t: any) => t.title).join(', ')}` : ''}
${todoContext.overdueTodos.length > 0 ? `- Overdue: ${todoContext.overdueTodos.map((t: any) => t.title).join(', ')}` : ''}
...
Generate ONLY the notification text, nothing else.`
    ...
    return completion.choices[0]?.message?.content || 'You\'ve got this! Stay focused 💚';
  } catch (error) {
...
async function generateEducationProgressMessage(educationContext: any): Promise<string> {
  try {
...
${educationContext.recentLessons.length > 0 ? `- Recent lessons: ${educationContext.recentLessons.map((l: any) => l.lessonId).join(', ')}` : ''}
` : '';
...
Generate ONLY the notification text, nothing else.`
    ...
    return completion.choices[0]?.message?.content || 'Your learning journey matters! Keep growing 💚';
  } catch (error) {
...
async function generateProgressReviewMessage(progressContext: any): Promise<string> {
  try {
...
Generate ONLY the notification text, nothing else.`
    ...
    return completion.choices[0]?.message?.content || 'Your progress matters! Keep going 💚';
  } catch (error) {
...
export const sendDailyProgressReview = functions.pubsub
...
export const sendDailyEducationProgress = functions.pubsub
...
export const sendDailyAccountabilityReminder = functions.pubsub
