...
class Lesson133Screen extends StatefulWidget {
  const Lesson133Screen({super.key});

  @override
  State<Lesson133Screen> createState() => _Lesson133ScreenState();
}

class _Lesson133ScreenState extends State<Lesson133Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(13, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 13.3: Debunking Diet Culture Myths'),
        ),
        body: const Center(
