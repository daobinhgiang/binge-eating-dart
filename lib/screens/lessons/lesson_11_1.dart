...
class Lesson111Screen extends StatefulWidget {
  const Lesson111Screen({super.key});

  @override
  State<Lesson111Screen> createState() => _Lesson111ScreenState();
}

class _Lesson111ScreenState extends State<Lesson111Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(11, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 11.1: Understanding Trauma and Eating Disorders'),
        ),
        body: const Center(
