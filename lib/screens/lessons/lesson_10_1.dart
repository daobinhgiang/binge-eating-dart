...
class Lesson101Screen extends StatefulWidget {
  const Lesson101Screen({super.key});

  @override
  State<Lesson101Screen> createState() => _Lesson101ScreenState();
}

class _Lesson101ScreenState extends State<Lesson101Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(10, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 10.1: Meal Planning and Preparation'),
        ),
        body: const Center(
