...
class Lesson182Screen extends StatefulWidget {
  const Lesson182Screen({super.key});

  @override
  State<Lesson182Screen> createState() => _Lesson182ScreenState();
}

class _Lesson182ScreenState extends State<Lesson182Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(18, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 18.2: Support Groups and Community'),
        ),
        body: const Center(
