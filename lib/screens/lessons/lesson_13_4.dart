...
class Lesson134Screen extends StatefulWidget {
  const Lesson134Screen({super.key});

  @override
  State<Lesson134Screen> createState() => _Lesson134ScreenState();
}

class _Lesson134ScreenState extends State<Lesson134Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(13, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 13.4: Working with a Registered Dietitian'),
        ),
        body: const Center(
