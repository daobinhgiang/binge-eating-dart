...
class Lesson95Screen extends StatefulWidget {
  const Lesson95Screen({super.key});

  @override
  State<Lesson95Screen> createState() => _Lesson95ScreenState();
}

class _Lesson95ScreenState extends State<Lesson95Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(9, 5);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 9.5: Building a Support Network'),
        ),
        body: const Center(
