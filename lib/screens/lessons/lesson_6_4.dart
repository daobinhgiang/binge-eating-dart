...
class Lesson64Screen extends StatefulWidget {
  const Lesson64Screen({super.key});

  @override
  State<Lesson64Screen> createState() => _Lesson64ScreenState();
}

class _Lesson64ScreenState extends State<Lesson64Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(6, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 6.4: Body Appreciation and Gratitude'),
        ),
        body: const Center(
