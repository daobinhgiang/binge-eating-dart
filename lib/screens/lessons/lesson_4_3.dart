...
class Lesson43Screen extends StatefulWidget {
  const Lesson43Screen({super.key});

  @override
  State<Lesson43Screen> createState() => _Lesson43ScreenState();
}

class _Lesson43ScreenState extends State<Lesson43Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(4, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 4.3: The STOP Technique'),
        ),
        body: const Center(
