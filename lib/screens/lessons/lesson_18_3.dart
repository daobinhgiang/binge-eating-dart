...
class Lesson183Screen extends StatefulWidget {
  const Lesson183Screen({super.key});

  @override
  State<Lesson183Screen> createState() => _Lesson183ScreenState();
}

class _Lesson183ScreenState extends State<Lesson183Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(18, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 18.3: Helpful Books and Resources'),
        ),
        body: const Center(
