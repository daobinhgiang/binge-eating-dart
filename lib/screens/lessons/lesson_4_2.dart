...
class Lesson42Screen extends StatefulWidget {
  const Lesson42Screen({super.key});

  @override
  State<Lesson42Screen> createState() => _Lesson42ScreenState();
}

class _Lesson42ScreenState extends State<Lesson42Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(4, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 4.2: Healthy Alternatives to Binge Eating'),
        ),
        body: const Center(
