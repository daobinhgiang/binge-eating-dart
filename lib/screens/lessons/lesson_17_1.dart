...
class Lesson171Screen extends StatefulWidget {
  const Lesson171Screen({super.key});

  @override
  State<Lesson171Screen> createState() => _Lesson171ScreenState();
}

class _Lesson171ScreenState extends State<Lesson171Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(17, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 17.1: Hope in Recovery'),
        ),
        body: const Center(
