...
class Lesson141Screen extends StatefulWidget {
  const Lesson141Screen({super.key});

  @override
  State<Lesson141Screen> createState() => _Lesson141ScreenState();
}

class _Lesson141ScreenState extends State<Lesson141Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(14, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 14.1: Healing Your Relationship with Exercise'),
        ),
        body: const Center(
