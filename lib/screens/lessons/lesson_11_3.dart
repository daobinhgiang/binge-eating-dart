...
class Lesson113Screen extends StatefulWidget {
  const Lesson113Screen({super.key});

  @override
  State<Lesson113Screen> createState() => _Lesson113ScreenState();
}

class _Lesson113ScreenState extends State<Lesson113Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(11, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 11.3: Working with Trauma Responses'),
        ),
        body: const Center(
