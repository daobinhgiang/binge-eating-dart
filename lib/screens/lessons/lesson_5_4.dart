...
class Lesson54Screen extends StatefulWidget {
  const Lesson54Screen({super.key});

  @override
  State<Lesson54Screen> createState() => _Lesson54ScreenState();
}

class _Lesson54ScreenState extends State<Lesson54Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(5, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 5.4: Dealing with Food Cravings'),
        ),
        body: const Center(
