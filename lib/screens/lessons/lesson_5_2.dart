...
class Lesson52Screen extends StatefulWidget {
  const Lesson52Screen({super.key});

  @override
  State<Lesson52Screen> createState() => _Lesson52ScreenState();
}

class _Lesson52ScreenState extends State<Lesson52Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(5, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 5.2: Making Peace with All Foods'),
        ),
        body: const Center(
