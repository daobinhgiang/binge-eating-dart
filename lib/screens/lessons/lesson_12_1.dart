...
class Lesson121Screen extends StatefulWidget {
  const Lesson121Screen({super.key});

  @override
  State<Lesson121Screen> createState() => _Lesson121ScreenState();
}

class _Lesson121ScreenState extends State<Lesson121Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(12, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 12.1: Depression and Eating Disorders'),
        ),
        body: const Center(
