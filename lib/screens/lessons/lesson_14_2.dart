...
class Lesson142Screen extends StatefulWidget {
  const Lesson142Screen({super.key});

  @override
  State<Lesson142Screen> createState() => _Lesson142ScreenState();
}

class _Lesson142ScreenState extends State<Lesson142Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(14, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 14.2: Movement for Mental Health'),
        ),
        body: const Center(
