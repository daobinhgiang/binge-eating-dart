...
class Lesson181Screen extends StatefulWidget {
  const Lesson181Screen({super.key});

  @override
  State<Lesson181Screen> createState() => _Lesson181ScreenState();
}

class _Lesson181ScreenState extends State<Lesson181Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(18, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 18.1: Professional Treatment Options'),
        ),
        body: const Center(
