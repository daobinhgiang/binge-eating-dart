...
class Lesson35Screen extends ConsumerStatefulWidget {
  const Lesson35Screen({super.key});

  @override
  ConsumerState<Lesson35Screen> createState() => _Lesson35ScreenState();
}

class _Lesson35ScreenState extends ConsumerState<Lesson35Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson35 = stage1.chapters[2].lessons[4]; // Chapter 3 (index 2), Lesson 5 (index 4)
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [Lesson35Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [Lesson35Screen] User not found');
        return;
      }
      ...
      if (result.questCompleted && mounted) {
        print('🎉 [Lesson35Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [Lesson35Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [Lesson35Screen] Error in quest completion: $e');
    }
  }
