...
class Lesson53Screen extends StatefulWidget {
  const Lesson53Screen({super.key});

  @override
  State<Lesson53Screen> createState() => _Lesson53ScreenState();
}

class _Lesson53ScreenState extends State<Lesson53Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(5, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 5.3: Intuitive Eating Principles'),
        ),
        body: const Center(
