...
class LessonS263Screen extends ConsumerStatefulWidget {
  const LessonS263Screen({super.key});

  @override
  ConsumerState<LessonS263Screen> createState() => _LessonS263ScreenState();
}

class _LessonS263ScreenState extends ConsumerState<LessonS263Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_6_3');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS263Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS263Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_6_3',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS263Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS263Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS263Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 6.3')),
