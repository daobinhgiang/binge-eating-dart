...
class Lesson65Screen extends StatefulWidget {
  const Lesson65Screen({super.key});

  @override
  State<Lesson65Screen> createState() => _Lesson65ScreenState();
}

class _Lesson65ScreenState extends State<Lesson65Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(6, 5);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 6.5: Dealing with Weight Stigma'),
        ),
        body: const Center(
