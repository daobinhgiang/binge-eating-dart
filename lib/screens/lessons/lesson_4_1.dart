...
class Lesson41Screen extends StatefulWidget {
  const Lesson41Screen({super.key});

  @override
  State<Lesson41Screen> createState() => _Lesson41ScreenState();
}

class _Lesson41ScreenState extends State<Lesson41Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(4, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 4.1: Identifying Your Coping Patterns'),
        ),
        body: const Center(
