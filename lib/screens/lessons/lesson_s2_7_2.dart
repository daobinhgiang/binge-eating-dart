...
class LessonS272Screen extends ConsumerStatefulWidget {
  const LessonS272Screen({super.key});

  @override
  ConsumerState<LessonS272Screen> createState() => _LessonS272ScreenState();
}

class _LessonS272ScreenState extends ConsumerState<LessonS272Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_7_2');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS272Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS272Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_7_2',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS272Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS272Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS272Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 7.2')),
