...
class LessonS223Screen extends ConsumerStatefulWidget {
  const LessonS223Screen({super.key});

  @override
  ConsumerState<LessonS223Screen> createState() => _LessonS223ScreenState();
}

class _LessonS223ScreenState extends ConsumerState<LessonS223Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_2_3');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS223Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS223Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_2_3',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS223Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS223Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS223Screen] Error in quest completion: $e');
    }
  }
