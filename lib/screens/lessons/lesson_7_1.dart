...
class Lesson71Screen extends StatefulWidget {
  const Lesson71Screen({super.key});

  @override
  State<Lesson71Screen> createState() => _Lesson71ScreenState();
}

class _Lesson71ScreenState extends State<Lesson71Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(7, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 7.1: Understanding Setbacks vs. Relapse'),
        ),
        body: const Center(
