...
class LessonS232Screen extends ConsumerStatefulWidget {
  const LessonS232Screen({super.key});

  @override
  ConsumerState<LessonS232Screen> createState() => _LessonS232ScreenState();
}

class _LessonS232ScreenState extends ConsumerState<LessonS232Screen> {
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS232Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS232Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_3_2',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS232Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS232Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS232Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 3.2')),
