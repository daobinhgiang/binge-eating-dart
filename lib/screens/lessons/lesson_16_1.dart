...
class Lesson161Screen extends StatefulWidget {
  const Lesson161Screen({super.key});

  @override
  State<Lesson161Screen> createState() => _Lesson161ScreenState();
}

class _Lesson161ScreenState extends State<Lesson161Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(16, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 16.1: Separating Identity from Eating Disorder'),
        ),
        body: const Center(
