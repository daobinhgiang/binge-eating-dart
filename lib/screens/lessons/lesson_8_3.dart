...
class Lesson83Screen extends StatefulWidget {
  const Lesson83Screen({super.key});

  @override
  State<Lesson83Screen> createState() => _Lesson83ScreenState();
}

class _Lesson83ScreenState extends State<Lesson83Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(8, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 8.3: Time Management and Boundaries'),
        ),
        body: const Center(
