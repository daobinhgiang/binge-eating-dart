...
class Lesson38Screen extends ConsumerStatefulWidget {
  const Lesson38Screen({super.key});

  @override
  ConsumerState<Lesson38Screen> createState() => _Lesson38ScreenState();
}

class _Lesson38ScreenState extends ConsumerState<Lesson38Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson38 = stage1.chapters[2].lessons[7]; // Chapter 3 (index 2), Lesson 8 (index 7)
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [Lesson38Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [Lesson38Screen] User not found');
        return;
      }
      ...
      if (result.questCompleted && mounted) {
        print('🎉 [Lesson38Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [Lesson38Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [Lesson38Screen] Error in quest completion: $e');
    }
  }
