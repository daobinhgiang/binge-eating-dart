...
class _Lesson12ScreenState extends ConsumerState<Lesson12Screen> {
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      ...
      if (user == null) {
        print('❌ [Lesson12Screen] User not found');
        return;
      }
      ...
      if (result.questCompleted && mounted) {
        print('🎉 [Lesson12Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [Lesson12Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [Lesson12Screen] Error in quest completion: $e');
    }
  }
