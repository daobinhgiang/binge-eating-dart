...
class Lesson66Screen extends StatefulWidget {
  const Lesson66Screen({super.key});

  @override
  State<Lesson66Screen> createState() => _Lesson66ScreenState();
}

class _Lesson66ScreenState extends State<Lesson66Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(6, 6);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 6.6: Developing Self-Compassion'),
        ),
        body: const Center(
