...
class LessonS226Screen extends ConsumerStatefulWidget {
  const LessonS226Screen({super.key});

  @override
  ConsumerState<LessonS226Screen> createState() => _LessonS226ScreenState();
}

class _LessonS226ScreenState extends ConsumerState<LessonS226Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_2_6');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS226Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS226Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_2_6',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS226Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS226Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS226Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 2.6')),
