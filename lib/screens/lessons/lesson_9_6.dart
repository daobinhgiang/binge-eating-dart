...
class Lesson96Screen extends StatefulWidget {
  const Lesson96Screen({super.key});

  @override
  State<Lesson96Screen> createState() => _Lesson96ScreenState();
}

class _Lesson96ScreenState extends State<Lesson96Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(9, 6);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 9.6: Communicating Your Needs'),
        ),
        body: const Center(
