...
class Lesson84Screen extends StatefulWidget {
  const Lesson84Screen({super.key});

  @override
  State<Lesson84Screen> createState() => _Lesson84ScreenState();
}

class _Lesson84ScreenState extends State<Lesson84Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(8, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 8.4: Building Stress Resilience'),
        ),
        body: const Center(
