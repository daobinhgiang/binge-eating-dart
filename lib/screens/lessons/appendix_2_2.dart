...
class Appendix22Screen extends StatefulWidget {
  const Appendix22Screen({super.key});

  @override
  State<Appendix22Screen> createState() => _Appendix22ScreenState();
}

class _Appendix22ScreenState extends State<Appendix22Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(102, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 2.2: Grocery Shopping and Food Preparation'),
        ),
        body: const Center(
