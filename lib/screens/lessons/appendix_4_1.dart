...
class Appendix41Screen extends StatefulWidget {
  const Appendix41Screen({super.key});

  @override
  State<Appendix41Screen> createState() => _Appendix41ScreenState();
}

class _Appendix41ScreenState extends State<Appendix41Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(104, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 4.1: How to Support a Loved One with an Eating Disorder'),
        ),
        body: const Center(
