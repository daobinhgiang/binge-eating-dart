...
class Lesson62Screen extends StatefulWidget {
  const Lesson62Screen({super.key});

  @override
  State<Lesson62Screen> createState() => _Lesson62ScreenState();
}

class _Lesson62ScreenState extends State<Lesson62Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(6, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 6.2: Challenging Negative Body Thoughts'),
        ),
        body: const Center(
