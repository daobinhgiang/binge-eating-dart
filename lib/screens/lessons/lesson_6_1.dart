...
class Lesson61Screen extends StatefulWidget {
  const Lesson61Screen({super.key});

  @override
  State<Lesson61Screen> createState() => _Lesson61ScreenState();
}

class _Lesson61ScreenState extends State<Lesson61Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(6, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 6.1: Understanding Body Image'),
        ),
        body: const Center(
