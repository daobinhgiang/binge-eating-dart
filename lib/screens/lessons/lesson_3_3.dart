...
class Lesson33Screen extends ConsumerStatefulWidget {
  const Lesson33Screen({super.key});

  @override
  ConsumerState<Lesson33Screen> createState() => _Lesson33ScreenState();
}

class _Lesson33ScreenState extends ConsumerState<Lesson33Screen> {
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [Lesson33Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [Lesson33Screen] User not found');
        return;
      }
      ...
      if (result.questCompleted && mounted) {
        print('🎉 [Lesson33Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [Lesson33Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [Lesson33Screen] Error in quest completion: $e');
    }
  }
