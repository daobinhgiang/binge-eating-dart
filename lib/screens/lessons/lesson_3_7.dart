...
class Lesson37Screen extends ConsumerStatefulWidget {
  const Lesson37Screen({super.key});

  @override
  ConsumerState<Lesson37Screen> createState() => _Lesson37ScreenState();
}

class _Lesson37ScreenState extends ConsumerState<Lesson37Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson37 = stage1.chapters[2].lessons[6]; // Chapter 3 (index 2), Lesson 7 (index 6)
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [Lesson37Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [Lesson37Screen] User not found');
        return;
      }
      ...
      if (result.questCompleted && mounted) {
        print('🎉 [Lesson37Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [Lesson37Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [Lesson37Screen] Error in quest completion: $e');
    }
  }
