...
class Lesson56Screen extends StatefulWidget {
  const Lesson56Screen({super.key});

  @override
  State<Lesson56Screen> createState() => _Lesson56ScreenState();
}

class _Lesson56ScreenState extends State<Lesson56Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(5, 6);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 5.6: Overcoming Food Guilt and Shame'),
        ),
        body: const Center(
