...
class Lesson151Screen extends StatefulWidget {
  const Lesson151Screen({super.key});

  @override
  State<Lesson151Screen> createState() => _Lesson151ScreenState();
}

class _Lesson151ScreenState extends State<Lesson151Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(15, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 15.1: Athletes and Eating Disorders'),
        ),
        body: const Center(
