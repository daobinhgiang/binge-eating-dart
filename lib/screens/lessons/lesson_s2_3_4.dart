...
class LessonS234Screen extends ConsumerStatefulWidget {
  const LessonS234Screen({super.key});

  @override
  ConsumerState<LessonS234Screen> createState() => _LessonS234ScreenState();
}

class _LessonS234ScreenState extends ConsumerState<LessonS234Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_3_4');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS234Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS234Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_3_4',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS234Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS234Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS234Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 3.4')),
