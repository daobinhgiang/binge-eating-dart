...
class Appendix32Screen extends StatefulWidget {
  const Appendix32Screen({super.key});

  @override
  State<Appendix32Screen> createState() => _Appendix32ScreenState();
}

class _Appendix32ScreenState extends State<Appendix32Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(103, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 3.2: Media Literacy and Social Media Guidelines'),
        ),
        body: const Center(
