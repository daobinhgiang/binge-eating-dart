...
class Appendix11Screen extends StatefulWidget {
  const Appendix11Screen({super.key});

  @override
  State<Appendix11Screen> createState() => _Appendix11ScreenState();
}

class _Appendix11ScreenState extends State<Appendix11Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(101, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 1.1: Crisis Support and Hotlines'),
        ),
        body: const Center(
