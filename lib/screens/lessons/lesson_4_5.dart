...
class Lesson45Screen extends StatefulWidget {
  const Lesson45Screen({super.key});

  @override
  State<Lesson45Screen> createState() => _Lesson45ScreenState();
}

class _Lesson45ScreenState extends State<Lesson45Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(4, 5);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 4.5: Building Your Support System'),
        ),
        body: const Center(
