...
class LessonS204Screen extends ConsumerStatefulWidget {
  const LessonS204Screen({super.key});

  @override
  ConsumerState<LessonS204Screen> createState() => _LessonS204ScreenState();
}

class _LessonS204ScreenState extends ConsumerState<LessonS204Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_0_4');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS204Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS204Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_0_4',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS204Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS204Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS204Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 0.4')),
