...
class Lesson55Screen extends StatefulWidget {
  const Lesson55Screen({super.key});

  @override
  State<Lesson55Screen> createState() => _Lesson55ScreenState();
}

class _Lesson55ScreenState extends State<Lesson55Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(5, 5);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 5.5: Eating for Energy and Mood'),
        ),
        body: const Center(
