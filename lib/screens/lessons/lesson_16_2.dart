...
class Lesson162Screen extends StatefulWidget {
  const Lesson162Screen({super.key});

  @override
  State<Lesson162Screen> createState() => _Lesson162ScreenState();
}

class _Lesson162ScreenState extends State<Lesson162Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(16, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 16.2: Building Self-Worth Beyond Appearance'),
        ),
        body: const Center(
