...
class Lesson143Screen extends StatefulWidget {
  const Lesson143Screen({super.key});

  @override
  State<Lesson143Screen> createState() => _Lesson143ScreenState();
}

class _Lesson143ScreenState extends State<Lesson143Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(14, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 14.3: Finding Activities You Enjoy'),
        ),
        body: const Center(
