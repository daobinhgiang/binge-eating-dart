...
class Lesson123Screen extends StatefulWidget {
  const Lesson123Screen({super.key});

  @override
  State<Lesson123Screen> createState() => _Lesson123ScreenState();
}

class _Lesson123ScreenState extends State<Lesson123Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(12, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 12.3: OCD and Eating Disorders'),
        ),
        body: const Center(
