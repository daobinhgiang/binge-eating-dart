...
class Lesson94Screen extends StatefulWidget {
  const Lesson94Screen({super.key});

  @override
  State<Lesson94Screen> createState() => _Lesson94ScreenState();
}

class _Lesson94ScreenState extends State<Lesson94Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(9, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 9.4: Dating and Relationships'),
        ),
        body: const Center(
