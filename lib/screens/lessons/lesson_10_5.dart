...
class Lesson105Screen extends StatefulWidget {
  const Lesson105Screen({super.key});

  @override
  State<Lesson105Screen> createState() => _Lesson105ScreenState();
}

class _Lesson105ScreenState extends State<Lesson105Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(10, 5);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 10.5: Creating Your Recovery Maintenance Plan'),
        ),
        body: const Center(
