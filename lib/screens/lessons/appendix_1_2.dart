...
class Appendix12Screen extends StatefulWidget {
  const Appendix12Screen({super.key});

  @override
  State<Appendix12Screen> createState() => _Appendix12ScreenState();
}

class _Appendix12ScreenState extends State<Appendix12Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(101, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 1.2: Medical Complications and Warning Signs'),
        ),
        body: const Center(
