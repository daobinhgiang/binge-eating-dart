...
class Lesson39Screen extends ConsumerStatefulWidget {
  const Lesson39Screen({super.key});

  @override
  ConsumerState<Lesson39Screen> createState() => _Lesson39ScreenState();
}

class _Lesson39ScreenState extends ConsumerState<Lesson39Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson39 = stage1.chapters[2].lessons[8]; // Chapter 3 (index 2), Lesson 9 (index 8)
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [Lesson39Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [Lesson39Screen] User not found');
        return;
      }
      ...
      if (result.questCompleted && mounted) {
        print('🎉 [Lesson39Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [Lesson39Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [Lesson39Screen] Error in quest completion: $e');
    }
  }
