...
class Appendix24Screen extends StatefulWidget {
  const Appendix24Screen({super.key});

  @override
  State<Appendix24Screen> createState() => _Appendix24ScreenState();
}

class _Appendix24ScreenState extends State<Appendix24Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(102, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 2.4: Special Dietary Considerations'),
        ),
        body: const Center(
