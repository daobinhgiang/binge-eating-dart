...
class Lesson172Screen extends StatefulWidget {
  const Lesson172Screen({super.key});

  @override
  State<Lesson172Screen> createState() => _Lesson172ScreenState();
}

class _Lesson172ScreenState extends State<Lesson172Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(17, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 17.2: Overcoming Common Recovery Challenges'),
        ),
        body: const Center(
