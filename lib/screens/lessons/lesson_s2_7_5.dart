...
class LessonS275Screen extends ConsumerStatefulWidget {
  const LessonS275Screen({super.key});

  @override
  ConsumerState<LessonS275Screen> createState() => _LessonS275ScreenState();
}

class _LessonS275ScreenState extends ConsumerState<LessonS275Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_7_5');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS275Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS275Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_7_5',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS275Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS275Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS275Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 7.5')),
