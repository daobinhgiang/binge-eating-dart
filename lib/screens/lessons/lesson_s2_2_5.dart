...
class LessonS225Screen extends ConsumerStatefulWidget {
  const LessonS225Screen({super.key});

  @override
  ConsumerState<LessonS225Screen> createState() => _LessonS225ScreenState();
}

class _LessonS225ScreenState extends ConsumerState<LessonS225Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_2_5');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS225Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS225Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_2_5',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS225Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS225Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS225Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 2.5')),
