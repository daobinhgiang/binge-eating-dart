...
class LessonS301Screen extends ConsumerStatefulWidget {
  const LessonS301Screen({super.key});

  @override
  ConsumerState<LessonS301Screen> createState() => _LessonS301ScreenState();
}

class _LessonS301ScreenState extends ConsumerState<LessonS301Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage3.chapters[0].lessons[0];
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS301Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS301Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s3_0_1',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS301Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS301Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS301Screen] Error in quest completion: $e');
    }
  }
