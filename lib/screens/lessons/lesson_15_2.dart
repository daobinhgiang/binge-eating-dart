...
class Lesson152Screen extends StatefulWidget {
  const Lesson152Screen({super.key});

  @override
  State<Lesson152Screen> createState() => _Lesson152ScreenState();
}

class _Lesson152ScreenState extends State<Lesson152Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(15, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 15.2: Eating Disorders in Midlife and Beyond'),
        ),
        body: const Center(
