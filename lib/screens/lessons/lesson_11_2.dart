...
class Lesson112Screen extends StatefulWidget {
  const Lesson112Screen({super.key});

  @override
  State<Lesson112Screen> createState() => _Lesson112ScreenState();
}

class _Lesson112ScreenState extends State<Lesson112Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(11, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 11.2: Building Safety and Grounding'),
        ),
        body: const Center(
