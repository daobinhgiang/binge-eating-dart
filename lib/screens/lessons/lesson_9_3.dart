...
class Lesson93Screen extends StatefulWidget {
  const Lesson93Screen({super.key});

  @override
  State<Lesson93Screen> createState() => _Lesson93ScreenState();
}

class _Lesson93ScreenState extends State<Lesson93Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(9, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 9.3: Family Dynamics and Food'),
        ),
        body: const Center(
