...
class Lesson91Screen extends StatefulWidget {
  const Lesson91Screen({super.key});

  @override
  State<Lesson91Screen> createState() => _Lesson91ScreenState();
}

class _Lesson91ScreenState extends State<Lesson91Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(9, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 9.1: Navigating Social Eating'),
        ),
        body: const Center(
