...
class Lesson104Screen extends StatefulWidget {
  const Lesson104Screen({super.key});

  @override
  State<Lesson104Screen> createState() => _Lesson104ScreenState();
}

class _Lesson104ScreenState extends State<Lesson104Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(10, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 10.4: Work and School Balance'),
        ),
        body: const Center(
