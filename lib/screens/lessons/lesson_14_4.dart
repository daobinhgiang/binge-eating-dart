...
class Lesson144Screen extends StatefulWidget {
  const Lesson144Screen({super.key});

  @override
  State<Lesson144Screen> createState() => _Lesson144ScreenState();
}

class _Lesson144ScreenState extends State<Lesson144Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(14, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 14.4: Movement Guidelines for Recovery'),
        ),
        body: const Center(
