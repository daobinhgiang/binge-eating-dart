...
class Lesson51Screen extends StatefulWidget {
  const Lesson51Screen({super.key});

  @override
  State<Lesson51Screen> createState() => _Lesson51ScreenState();
}

class _Lesson51ScreenState extends State<Lesson51Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(5, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 5.1: Food Rules vs. Food Freedom'),
        ),
        body: const Center(
