...
class Lesson36Screen extends ConsumerStatefulWidget {
  const Lesson36Screen({super.key});

  @override
  ConsumerState<Lesson36Screen> createState() => _Lesson36ScreenState();
}

class _Lesson36ScreenState extends ConsumerState<Lesson36Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson36 = stage1.chapters[2].lessons[5]; // Chapter 3 (index 2), Lesson 6 (index 5)
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [Lesson36Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [Lesson36Screen] User not found');
        return;
      }
      ...
      if (result.questCompleted && mounted) {
        print('🎉 [Lesson36Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [Lesson36Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [Lesson36Screen] Error in quest completion: $e');
    }
  }
