...
class Appendix13Screen extends StatefulWidget {
  const Appendix13Screen({super.key});

  @override
  State<Appendix13Screen> createState() => _Appendix13ScreenState();
}

class _Appendix13ScreenState extends State<Appendix13Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(101, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 1.3: Safety Planning for High-Risk Situations'),
        ),
        body: const Center(
