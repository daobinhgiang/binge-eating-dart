...
class LessonS202Screen extends ConsumerStatefulWidget {
  const LessonS202Screen({super.key});

  @override
  ConsumerState<LessonS202Screen> createState() => _LessonS202ScreenState();
}

class _LessonS202ScreenState extends ConsumerState<LessonS202Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_0_2');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS202Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS202Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_0_2',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS202Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS202Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS202Screen] Error in quest completion: $e');
    }
  }
