...
class Lesson102Screen extends StatefulWidget {
  const Lesson102Screen({super.key});

  @override
  State<Lesson102Screen> createState() => _Lesson102ScreenState();
}

class _Lesson102ScreenState extends State<Lesson102Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(10, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 10.2: Exercise and Movement in Recovery'),
        ),
        body: const Center(
