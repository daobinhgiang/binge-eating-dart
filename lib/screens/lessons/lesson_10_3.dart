...
class Lesson103Screen extends StatefulWidget {
  const Lesson103Screen({super.key});

  @override
  State<Lesson103Screen> createState() => _Lesson103ScreenState();
}

class _Lesson103ScreenState extends State<Lesson103Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(10, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 10.3: Sleep and Recovery'),
        ),
        body: const Center(
