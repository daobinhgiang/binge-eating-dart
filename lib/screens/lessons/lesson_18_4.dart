...
class Lesson184Screen extends StatefulWidget {
  const Lesson184Screen({super.key});

  @override
  State<Lesson184Screen> createState() => _Lesson184ScreenState();
}

class _Lesson184ScreenState extends State<Lesson184Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(18, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 18.4: Creating Your Personal Recovery Plan'),
        ),
        body: const Center(
