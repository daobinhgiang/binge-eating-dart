...
class Lesson173Screen extends StatefulWidget {
  const Lesson173Screen({super.key});

  @override
  State<Lesson173Screen> createState() => _Lesson173ScreenState();
}

class _Lesson173ScreenState extends State<Lesson173Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(17, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 17.3: Life After Recovery'),
        ),
        body: const Center(
