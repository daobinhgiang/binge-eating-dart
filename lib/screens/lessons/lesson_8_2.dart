...
class Lesson82Screen extends StatefulWidget {
  const Lesson82Screen({super.key});

  @override
  State<Lesson82Screen> createState() => _Lesson82ScreenState();
}

class _Lesson82ScreenState extends State<Lesson82Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(8, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 8.2: Breathing and Relaxation Techniques'),
        ),
        body: const Center(
