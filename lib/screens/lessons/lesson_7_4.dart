...
class Lesson74Screen extends StatefulWidget {
  const Lesson74Screen({super.key});

  @override
  State<Lesson74Screen> createState() => _Lesson74ScreenState();
}

class _Lesson74ScreenState extends State<Lesson74Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(7, 4);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 7.4: Building Long-Term Recovery'),
        ),
        body: const Center(
