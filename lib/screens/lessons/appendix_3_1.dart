...
class Appendix31Screen extends StatefulWidget {
  const Appendix31Screen({super.key});

  @override
  State<Appendix31Screen> createState() => _Appendix31ScreenState();
}

class _Appendix31ScreenState extends State<Appendix31Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(103, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 3.1: Body Neutrality and Acceptance Exercises'),
        ),
        body: const Center(
