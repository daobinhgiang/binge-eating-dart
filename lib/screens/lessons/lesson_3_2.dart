...
class Lesson32Screen extends ConsumerStatefulWidget {
  const Lesson32Screen({super.key});

  @override
  ConsumerState<Lesson32Screen> createState() => _Lesson32ScreenState();
}

class _Lesson32ScreenState extends ConsumerState<Lesson32Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson32 = stage1.chapters[2].lessons[1]; // Chapter 3 (index 2), Lesson 2 (index 1)
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [Lesson32Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [Lesson32Screen] User not found');
        return;
      }
      ...
      if (result.questCompleted && mounted) {
        print('🎉 [Lesson32Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [Lesson32Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [Lesson32Screen] Error in quest completion: $e');
    }
  }
