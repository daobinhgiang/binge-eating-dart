...
class Lesson122Screen extends StatefulWidget {
  const Lesson122Screen({super.key});

  @override
  State<Lesson122Screen> createState() => _Lesson122ScreenState();
}

class _Lesson122ScreenState extends State<Lesson122Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(12, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 12.2: Anxiety and Eating Disorders'),
        ),
        body: const Center(
