...
class Lesson73Screen extends StatefulWidget {
  const Lesson73Screen({super.key});

  @override
  State<Lesson73Screen> createState() => _Lesson73ScreenState();
}

class _Lesson73ScreenState extends State<Lesson73Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(7, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 7.3: High-Risk Situations'),
        ),
        body: const Center(
