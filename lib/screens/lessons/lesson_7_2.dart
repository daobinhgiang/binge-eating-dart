...
class Lesson72Screen extends StatefulWidget {
  const Lesson72Screen({super.key});

  @override
  State<Lesson72Screen> createState() => _Lesson72ScreenState();
}

class _Lesson72ScreenState extends State<Lesson72Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(7, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 7.2: Creating Your Relapse Prevention Plan'),
        ),
        body: const Center(
