...
class Lesson132Screen extends StatefulWidget {
  const Lesson132Screen({super.key});

  @override
  State<Lesson132Screen> createState() => _Lesson132ScreenState();
}

class _Lesson132ScreenState extends State<Lesson132Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(13, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 13.2: Understanding Your Body\'s Needs'),
        ),
        body: const Center(
