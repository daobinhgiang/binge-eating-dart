...
class LessonS212Screen extends ConsumerStatefulWidget {
  const LessonS212Screen({super.key});

  @override
  ConsumerState<LessonS212Screen> createState() => _LessonS212ScreenState();
}

class _LessonS212ScreenState extends ConsumerState<LessonS212Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_1_2');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS212Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS212Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_1_2',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS212Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS212Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS212Screen] Error in quest completion: $e');
    }
  }
