...
class Lesson81Screen extends StatefulWidget {
  const Lesson81Screen({super.key});

  @override
  State<Lesson81Screen> createState() => _Lesson81ScreenState();
}

class _Lesson81ScreenState extends State<Lesson81Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(8, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 8.1: Understanding Stress and Eating'),
        ),
        body: const Center(
