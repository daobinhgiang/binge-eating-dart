...
class LessonS278Screen extends ConsumerStatefulWidget {
  const LessonS278Screen({super.key});

  @override
  ConsumerState<LessonS278Screen> createState() => _LessonS278ScreenState();
}

class _LessonS278ScreenState extends ConsumerState<LessonS278Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_7_8');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS278Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS278Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_7_8',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS278Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS278Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS278Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 7.8')),
