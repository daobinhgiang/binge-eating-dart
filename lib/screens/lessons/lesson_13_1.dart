...
class Lesson131Screen extends StatefulWidget {
  const Lesson131Screen({super.key});

  @override
  State<Lesson131Screen> createState() => _Lesson131ScreenState();
}

class _Lesson131ScreenState extends State<Lesson131Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(13, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 13.1: Nutrition Basics for Recovery'),
        ),
        body: const Center(
