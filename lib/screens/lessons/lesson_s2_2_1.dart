...
class LessonS221Screen extends ConsumerStatefulWidget {
  const LessonS221Screen({super.key});

  @override
  ConsumerState<LessonS221Screen> createState() => _LessonS221ScreenState();
}

class _LessonS221ScreenState extends ConsumerState<LessonS221Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_2_1');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS221Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS221Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_2_1',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS221Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS221Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS221Screen] Error in quest completion: $e');
    }
  }
