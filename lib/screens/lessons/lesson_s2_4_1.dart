...
class LessonS241Screen extends ConsumerStatefulWidget {
  const LessonS241Screen({super.key});

  @override
  ConsumerState<LessonS241Screen> createState() => _LessonS241ScreenState();
}

class _LessonS241ScreenState extends ConsumerState<LessonS241Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_4_1');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS241Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS241Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_4_1',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS241Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS241Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS241Screen] Error in quest completion: $e');
    }
  }
