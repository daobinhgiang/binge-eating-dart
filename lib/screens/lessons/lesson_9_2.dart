...
class Lesson92Screen extends StatefulWidget {
  const Lesson92Screen({super.key});

  @override
  State<Lesson92Screen> createState() => _Lesson92ScreenState();
}

class _Lesson92ScreenState extends State<Lesson92Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(9, 2);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 9.2: Dealing with Food Comments'),
        ),
        body: const Center(
