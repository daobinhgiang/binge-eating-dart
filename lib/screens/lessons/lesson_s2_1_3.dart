...
class LessonS213Screen extends ConsumerStatefulWidget {
  const LessonS213Screen({super.key});

  @override
  ConsumerState<LessonS213Screen> createState() => _LessonS213ScreenState();
}

class _LessonS213ScreenState extends ConsumerState<LessonS213Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_1_3');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS213Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS213Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_1_3',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS213Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS213Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS213Screen] Error in quest completion: $e');
    }
  }
