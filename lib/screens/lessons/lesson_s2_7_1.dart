...
class LessonS271Screen extends ConsumerStatefulWidget {
  const LessonS271Screen({super.key});

  @override
  ConsumerState<LessonS271Screen> createState() => _LessonS271ScreenState();
}

class _LessonS271ScreenState extends ConsumerState<LessonS271Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters[7].lessons[0];
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS271Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS271Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_7_1',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS271Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS271Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS271Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 7.1')),
