...
class Lesson63Screen extends StatefulWidget {
  const Lesson63Screen({super.key});

  @override
  State<Lesson63Screen> createState() => _Lesson63ScreenState();
}

class _Lesson63ScreenState extends State<Lesson63Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(6, 3);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Lesson 6.3: Media Literacy and Body Image'),
        ),
        body: const Center(
