...
class LessonS233Screen extends ConsumerStatefulWidget {
  const LessonS233Screen({super.key});

  @override
  ConsumerState<LessonS233Screen> createState() => _LessonS233ScreenState();
}

class _LessonS233ScreenState extends ConsumerState<LessonS233Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      ...
      final lesson = stage2.chapters
          ...
          .lessons
          .firstWhere((lesson) => lesson.id == 'lesson_s2_3_3');
  ...
  Future<void> _handleLessonCompletion() async {
    try {
      print('\n📚 [LessonS233Screen] Calling quest completion handler...');
      final user = ref.read(currentUserDataProvider);
      if (user == null) {
        print('❌ [LessonS233Screen] User not found');
        return;
      }
      ...
      final result = await questCompletionService.handleLessonCompletion(
        userId: user.id,
        lessonId: 'lesson_s2_3_3',
      );
      
      if (result.questCompleted && mounted) {
        print('🎉 [LessonS233Screen] Quest completed! Showing dialog...');
        showQuestCompletionDialog(context, result);
      } else {
        print('ℹ️  [LessonS233Screen] No quest completed yet');
      }
    } catch (e) {
      print('❌ [LessonS233Screen] Error in quest completion: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(title: const Text('Lesson 3.3')),
