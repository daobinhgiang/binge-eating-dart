...
class Appendix21Screen extends StatefulWidget {
  const Appendix21Screen({super.key});

  @override
  State<Appendix21Screen> createState() => _Appendix21ScreenState();
}

class _Appendix21ScreenState extends State<Appendix21Screen> {
  ...
  Future<void> _loadLesson() async {
    try {
      final lesson = await _lessonService.getLessonByChapterAndNumber(102, 1);
      if (lesson != null) {
        setState(() {
          _lesson = lesson;
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    if (_lesson == null || _lesson!.slides.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Appendix 2.1: Sample Meal Plans for Recovery'),
        ),
        body: const Center(
